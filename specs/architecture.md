...
<!-- player-mix-controls SESSION-01 (2026-08-21 — appended by Jikijitsu) -->
...
<!-- playtest-ux-hotfix-batch SESSION-04 (2026-08-21 — appended by Jikijitsu) -->
### Visual — wide-dock EQUIP reachability + player HP off enemy-red (playtest-ux-hotfix-batch SESSION-04)

**M79 Components CSS / M101 Wide CSS (ui):** `.inventory-list` is now `flex: 0 0 auto; margin-bottom: 12px` (was `flex: 1; min-height: 0`). Portrait was unaffected (`.console-content` is a plain `overflow-y:auto` block). In the wide dock, `.wide-console-content-body` (`display:flex; flex-direction:column; overflow-y:auto`) collapsed a `flex:1` (basis 0) child to ~0 height when the equipped section filled the box, hiding every `EQUIP`/`TAG JUNK` row rendered by `gear.js` `renderInventoryItem`. Natural-height flow lets the whole list scroll inside the dock body. `wide.css` needed no diff (existing `.equipped-section`/`.inventory-list` margins + `padding: 12px 12px 24px` gutter still correct).
