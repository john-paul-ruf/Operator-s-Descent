...
| 02 | Single native control activation | M57, M61, M95 | `./src/ui/input.js`, `./tests/ui/input.test.js` | done | 2/2 | 2026-08-23 | Removed duplicate touchend activation from bindActionControl in ./src/ui/input.js, leaving native click as the sole control-activation path; source label now generic 'control'. Test rewritten to prove a stray touchend is inert, the following click fires exactly once, keyboard dispatch stays distinct/equivalent, and destroy() tears down the click listener. |
| 03 | Share sheet and haptic preference | M45, M73, M76 | `./src/state/library.js`, `./src/ui/screens/settings.js`, `./src/ui/screens/scorecard.js`, `./tests/state/library.test.js`, `./tests/ui/persistence-screens.test.js` | done | 2/2 | 2026-08-23 | hapticsEnabled durable setting (default false) added to M45/M76; M73 scorecard world-link now shares natively (navigator.share) with honest clipboard/select fallback, cancellation never falls through, testid preserved, label now SHARE WORLD LINK. |
| 04 | Combat haptic feedback | M45, M71 | `./src/ui/screens/combat.js`, `./tests/ui/combat-screen.test.js` | done | 2/2 | 2026-08-23 | hapticPatternForCombatEntry (exported, pure) maps resolved combat-log entries to Vibration API patterns: hit→12, crit→[12,24,24], death→[24,24,44], else null. Guarded emitter reads hapticsEnabled once at mount and fires exactly once per entry from dispatchLogEntry, the single point every resolved entry passes through across all dispatch paths. |
...
### SESSION-02 — 2026-08-23

- **Notes:** Removed duplicate touchend activation from bindActionControl in ./src/ui/input.js, leaving native click as the sole control-activation path; source label now generic 'control'. Test rewritten to prove a stray touchend is inert, the following click fires exactly once, keyboard dispatch stays distinct/equivalent, and destroy() tears down the click listener.
- **Delivered:** ./src/ui/input.js: bindActionControl binds one 'click' listener (no 'touchend'), symmetric cleanup, source: 'control'. ./tests/ui/input.test.js: touch-control test rewritten to model the touchend-then-click synthetic sequence and pin the teardown contract.
- **Verification:** node --check ./src/ui/input.js passed. npx vitest run ./tests/ui/input.test.js: 4/4 pass, run twice consecutively. git diff --check on lease paths: clean. Full npx vitest run: 2859/2861 pass — the 2 failures (tests/ui/exploration-screen.test.js: NO PATH / hostile-interrupt tap tests) are outside the lease; confirmed pre-existing by temporarily reverting ./src/ui/input.js to its pre-session-02 version and re-running — same 2 failures reproduce, unrelated to this change (playfield tap-path/hostile-interrupt logic, not bindActionControl). git diff <ckpt1>^..HEAD self-review: diff does exactly what SESSION-02 asked, nothing more. Read ./src/ui/console/move.js — its native <button> caller needs no change (already click-based).
- **Surprises:** tests/ui/exploration-screen.test.js has 2 pre-existing failures ('tap on an unreachable cell posts NO PATH...' and 'tap-to-move truncates on hostile interrupt') unrelated to this session's lease — confirmed by reproducing them against the pre-session-02 ./src/ui/input.js. Left untouched per orchestration rule 6/7 (not my lease, don't fix other sessions' in-flight work).
- **Follow-up:** —

### SESSION-03 — 2026-08-23

- **Notes:** hapticsEnabled durable setting (default false) added to M45/M76; M73 scorecard world-link now shares natively (navigator.share) with honest clipboard/select fallback, cancellation never falls through, testid preserved, label now SHARE WORLD LINK.
- **Delivered:** Checkpoint 1: hapticsEnabled default+normalization in ./src/state/library.js and a HAPTIC FEEDBACK toggle (data-testid=settings-haptics) in ./src/ui/screens/settings.js, persisted via the existing saveSettings/saveCurrent flow (no bus dispatch — no live-preview consumer exists yet; SESSION-04 reads it via loadSettings() at mount). Checkpoint 2: ./src/ui/screens/scorecard.js's world-link action now calls navigator.share() directly in the click gesture with the exact seed-only URL; success shows 'WORLD LINK SHARED', AbortError shows 'SHARE CANCELLED' without touching the clipboard, and API-absent/non-abort failures fall back to the pre-existing clipboard/select behavior unchanged. Visible/aria label is now 'SHARE WORLD LINK'; data-testid='scorecard-copy-world' preserved.
- **Verification:** npx vitest run ./tests/state/library.test.js ./tests/ui/persistence-screens.test.js → 45 pass (both checkpoints); npx vitest run ./tests/ui/front-door.test.js → 9 pass (settings screen regression check, out-of-lease file, unmodified); full npx vitest run → 2862 pass / 5 fail, all 5 failures in tests/integration/service-worker.test.js and tests/ui/exploration-screen.test.js, neither in this session's lease nor touched by this session's diff; npm run check:assets → passes (transfer budget 369734/512000 brotli bytes).
- **Surprises:** 5 pre-existing test failures outside my lease (tests/integration/service-worker.test.js — a fetch-mock assertion; tests/ui/exploration-screen.test.js — two NO PATH/hostile-interrupt assertions) — unrelated to hapticsEnabled/scorecard-share changes, not caused by this session's diff, and untouched files. Also observed substantial uncommitted concurrent work outside my lease (index.html, service-worker.js, scripts/server.js, scripts/verify-assets.js, scripts/report-budget.js, tools/serve.mjs, manifest.webmanifest, assets/app-icon*.png, tests/integration/service-worker.test.js) — left entirely untouched per lease discipline, presumably SESSION-01 in flight.
- **Follow-up:** SESSION-04 (combat haptics) should read `loadSettings().hapticsEnabled` at ./src/ui/screens/combat.js mount time — no bus event exists for this setting today. If a live-preview need ever appears, 'haptics' would need to be added to bus.js's SETTING_KEYS whitelist (out of this session's lease).

### SESSION-04 — 2026-08-23

- **Notes:** hapticPatternForCombatEntry (exported, pure) maps resolved combat-log entries to Vibration API patterns: hit→12, crit→[12,24,24], death→[24,24,44], else null. Guarded emitter reads hapticsEnabled once at mount and fires exactly once per entry from dispatchLogEntry, the single point every resolved entry passes through across all dispatch paths.
- **Delivered:** src/ui/screens/combat.js: exported hapticPatternForCombatEntry(entry) pure mapper; mount() reads settings.hapticsEnabled once (alongside the existing reducedMotion read); mount-scoped triggerHaptic(entry) guarded emitter (opt-in check, null-pattern check, navigator.vibrate existence check, try/catch around the call) wired into dispatchLogEntry — the sole existing dispatch point for party-immediate, paced-playback, fast-forward, reduced-motion-instant, and unmount-flush log entries, so no entry can double-fire. tests/ui/combat-screen.test.js: pattern-mapping unit tests, opt-in-guard tests (disabled preference, absent API, throwing API — all silent no-ops that never block combat resolution), and exactly-once behavioral tests across every dispatch path (immediate hit+death, clean miss via a seed search that avoids both fumble-retaliation and auto-crit, non-combat move/end-turn, paced playback, fast-forward, unmount mid-playback flush, reduced motion) asserting call arguments literally match hapticPatternForCombatEntry's own output.
- **Verification:** npx vitest run ./tests/ui/combat-screen.test.js → 75/75 pass (run twice, stable); npm run check:assets → 123 manifest assets, 371794 brotli bytes under the 512000 budget; node --check on both touched files passed; full npx vitest run → 2902/2906 pass, all 4 failures outside this session's lease and pre-existing (confirmed identical to prior sessions' documented surprises, unrelated to combat.js/combat-screen.test.js); git diff self-review confirms the change is scoped exactly to the mapping/emitter/wiring plus tests, nothing else touched.
- **Surprises:** A nat-1 fumble in this ruleset triggers an immediate enemy retaliation attack (a second, genuinely resolved 'attack' log entry) — an initial 'guaranteed miss via fumble' test fixture was wrong because it produced a real enemy hit and correctly vibrated once for it. Replaced with a weak-weapon/high-defense fixture plus a seed search that excludes both nat-1 and nat-20 rolls, giving a clean single miss. Also: `globalThis.navigator` is a getter-only global in this test runtime (Node 22's built-in Navigator), so tests must use `vi.stubGlobal('navigator', ...)`/`vi.unstubAllGlobals()` rather than direct assignment. Pre-existing, out-of-lease test failures (documented by SESSION-01/02/03): tests/tooling/check-tokens.test.js (safe-area tokens missing from specs/design.md), tests/ui/exploration-screen.test.js (2 NO PATH/hostile-interrupt tap tests), tests/ui/front-door.test.js (RELOAD-button contract change from SESSION-01) — none touched, none related to this session's diff.
- **Follow-up:** SESSION-05 (final offline cache release) can proceed — this session's lease is fully committed and clean. No new module was added (M71 gained one exported pure function on an existing file), so the arch delta is a small addition to the existing M71 section; fragment written to .forge/signal/SESSION-04.arch.md per the envelope, not committed.
