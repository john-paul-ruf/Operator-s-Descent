...
// depended on when RUN_SCHEMA_VERSION was 4. This file must NOT import
// the live save-codecs.js or save-schema.js — they will fork to v5+ in
// later sessions, and this reader must remain byte-identical to what
// v4 saves were encoded against forever.
//
// The one seam we keep to condense.js is readSymbol(reader, field, raw,
// version) — the v4 reader always passes the frozen table version (V4_TABLE_VERSION)
// so it selects the v4-snapshot table regardless of what the current table is.

import { readSymbol } from '../condense.js';

export const V4_TABLE_VERSION = 1;
...
function readField(reader, field, raw) {
  return readSymbol(reader, field, raw, V4_TABLE_VERSION);
}
...
// Frozen sigil-pool snapshot: pins the values in effect when v4 shipped so any
// future rebalance of data/enemies.json cannot silently invalidate a v4 save.
// The live pool in save-codecs.js may drift on future schema bumps; this copy
// stays put forever. See Custom Rule 13.
