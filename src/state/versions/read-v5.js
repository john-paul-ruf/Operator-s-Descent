...
// Self-contained payload reader for RUN_SCHEMA_VERSION = 5 saves.
// Byte-copy of decodeRunPayload from save-schema.js as it was pinned when
// v5 shipped. It intentionally does NOT import the live save-schema.js or
// save-codecs.js — those will fork to v6+ in later sessions, and this
// reader must remain byte-identical to what v5 saves were encoded against
...
// Reads a v5 payload. Assumes the frozen v5 symbol table (V5_TABLE_VERSION)
// is registered in the condenser registry — the caller (save-decode.js
// dispatch) guarantees this because v5 shares symbol-table version 1 with
// v3/v4 and initCondenser registers version 1 at boot.
export function readV5Payload(bytes, bitLength) {
  try {
    ...
    if (schemaVersion !== V5_SCHEMA_VERSION || tableVersion !== V5_TABLE_VERSION) fail('version_mismatch');
    const state = {
      worldSeed,
    ...
    state.themesSeen = Array.from({ length: themeLength }, () => readSymbol(reader, 'theme_id', () => readString(reader, 64), V5_TABLE_VERSION));
    ...
    state.flags = { version: V5_SCHEMA_VERSION, calibrationFloorsReached: Array.from({ length: calibrationLength }, () => integer(reader.readUint(8), 1, MAX_DEPTH, 'invalid_flags')) };
