// FROZEN — reads schema v6 forever; never edit.
// Self-contained payload reader for RUN_SCHEMA_VERSION = 6 saves.
// Byte-copy of decodeRunPayload from save-schema.js as it was pinned when
// v6 shipped. It intentionally does NOT import the live save-schema.js or
// save-codecs.js — those will fork to v7+ in later sessions, and this
// reader must remain byte-identical to what v6 saves were encoded against
...
// Reads a v6 payload. Assumes the frozen v6 symbol table (V6_TABLE_VERSION)
// is registered in the condenser registry — the caller (save-decode.js
// dispatch) guarantees this. v6 shares symbol-table version 1 with v3/v4/v5,
// which initCondenser registers at boot; once table v2 lands, save-decode
// wires `registerCondenserTable` for the frozen v1 snapshot so v6 frames
// still expand.
export function readV6Payload(bytes, bitLength) {
  try {
    ...
    if (schemaVersion !== V6_SCHEMA_VERSION || tableVersion !== V6_TABLE_VERSION) fail('version_mismatch');
    const state = {
      worldSeed,
    ...
    state.themesSeen = Array.from({ length: themeLength }, () => readSymbol(reader, 'theme_id', () => readString(reader, 64), V6_TABLE_VERSION));
    ...
    state.flags = { version: V6_SCHEMA_VERSION, calibrationFloorsReached: Array.from({ length: calibrationLength }, () => integer(reader.readUint(8), 1, MAX_DEPTH, 'invalid_flags')) };
