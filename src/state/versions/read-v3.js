...
// Self-contained payload reader for RUN_SCHEMA_VERSION = 3 saves.
// Byte-copy of decodeRunPayload from save-schema.js as it was pinned when
// v3 shipped. It intentionally does NOT import the live save-schema.js or
// save-codecs.js — those will fork to v4+ in later sessions, and this
// reader must remain byte-identical to what v3 saves were encoded against
...
// Reads a v3 payload. Assumes the frozen v3 symbol table (V3_TABLE_VERSION)
// is registered in the condenser registry — the caller (save-decode.js
// dispatch) guarantees this by calling registerV3SnapshotTable() at import.
export function readV3Payload(bytes, bitLength) {
  try {
    ...
    if (schemaVersion !== V3_SCHEMA_VERSION || tableVersion !== V3_TABLE_VERSION) fail('version_mismatch');
    const state = {
      worldSeed,
    ...
    state.themesSeen = Array.from({ length: themeLength }, () => readSymbol(reader, 'theme_id', () => readString(reader, 64), V3_TABLE_VERSION));
    ...
    state.flags = { version: V3_SCHEMA_VERSION, calibrationFloorsReached: Array.from({ length: calibrationLength }, () => integer(reader.readUint(8), 1, MAX_DEPTH, 'invalid_flags')) };
