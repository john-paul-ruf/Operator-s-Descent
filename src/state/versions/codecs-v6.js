...
// depended on when RUN_SCHEMA_VERSION was 6. This file must NOT import
// the live save-codecs.js or save-schema.js — those will fork to v7+ in
// later sessions, and this reader must remain byte-identical to what
// v6 saves were encoded against forever.
//
// The one seam we keep to condense.js is readSymbol(reader, field, raw,
// version) — the v6 reader always passes the frozen table version
// (V6_TABLE_VERSION = 1) so it selects the v6-snapshot table regardless
// of what the current table is.

import { readSymbol } from '../condense.js';

export const V6_TABLE_VERSION = 1;
...
function readField(reader, field, raw) {
  return readSymbol(reader, field, raw, V6_TABLE_VERSION);
}
