export function createRunState(worldSeed, party) {
  return {
    ...
    echoQueue: [],
    rngState: null,
    flags: { version: 1, calibrationFloorsReached: 0 },
    serialize() {
      return {
    ...
    queueEcho(echo) {
      if (this.echoQueue.length < 2) this.echoQueue.push(echo);
    },
    getDangerClockRate() {
      return 0.01;
    },
...
export function deserializeRunState(data) {
  ...
  if (data.fogOfWar) {
    state.fogOfWar = new Uint8Array(data.fogOfWar);
  }
  if (data.openedContainers) state.openedContainers = BigInt(data.openedContainers);
  if (data.defeatedEnemies) state.defeatedEnemies = BigInt(data.defeatedEnemies);
  if (data.dangerClockProgress) state.dangerClockProgress = data.dangerClockProgress;
  if (data.inventory) state.inventory = data.inventory;
  if (data.corruption) state.corruption = data.corruption;
  if (data.credits) state.credits = data.credits;
  if (data.scrapCounter) state.scrapCounter = data.scrapCounter;
