...
## clarity-and-fit SESSION-01 — Rev 2.2 (owner sign-off feedback: "sigils are v centered but not horizontally centered")

- **Root cause, metric (not geometric)**: `tools/font/build_font.py` set every glyph's `hmtx` entry to `(ADVANCE, 0)` — advance 1000 and left side bearing 0. TrueType rasterizers interpret `lsb=0` as "phantom point 0 lands at `xMin`", which shifts each outline left by `xMin` so ink starts at x=0 inside the 1000-unit advance box. Every glyph rendered left-aligned regardless of `textAlign: 'center'` on the canvas or the contact sheet's `place-items: center` — those center the *advance box*, not the ink. Vertical placement has no analogous shift, so REPAIR-02's y-centering (`CENTER_Y = 401`) still showed through correctly.
- **Fix, metric layer**: `setupHorizontalMetrics` now emits `(ADVANCE, glyph.xMin)` per glyph — the "identity" LSB where phantom point 0 lands at the origin and no outline shift occurs. Because rev-2.1 recipes are geometrically centered on `CENTER_X = 500` (= ADVANCE/2), symmetric glyphs now render with ink centered in their advance box. `.notdef` gets `lsb=180` (its actual xMin, from the rectangular frame at x=180..820). Empty glyphs would fall back to `lsb=0`, but the font has none — every mapped codepoint has real contours.
- **Verifier hardening (`scripts/verify-font.py`)**: two new per-glyph assertions.
  - `hmtx[name][1] == glyf[name].xMin` — metric honesty. Guards against a regression to `lsb=0` (or any other constant) for a single glyph.
  - `abs(((xMin + xMax) / 2) - ADVANCE / 2) <= INK_CENTER_TOLERANCE` where `INK_CENTER_TOLERANCE = 100` font units (10% of advance). See tolerance discussion below.
- **Tolerance discussion (why 100, not the 4% the fix originally targeted)**: three glyphs' ink centers land outside ±40 units by design and were audited before the tolerance was widened. All three are motif-intentional asymmetric compositions from the rev-2 grammar, not recipe drift:
  - **U+E043 `phantom_2` (bestiary phantom)** — center 587.5, offset **+87.5** (worst case). Three same-radius arcs from centers at cx = −80, 0, +80 all sweeping start=260° / sweep=200°. The sweep covers the right and bottom halves, so each arc's ink bulges rightward; the ensemble echo pattern is intrinsically right-heavy. Matches the SESSION-33 Phantom trio motif ("displaced double halos, unresolved contours, and offset echo traces").
  - **U+E025 `oracle_6` (player oracle)** — center 458.0, offset **−42.0**. The Oracle rev-2 motif is "one to four offset arc halos around off-center points" (control-and-polish SESSION-04 review). Small left-lean from the halo placement.
  - **U+E02E `operator_7` (player operator)** — center 541.0, offset **+41.0**. Operator's rev-2 grammar packs five voices per mark (ring fragment, polyline return, secondary branch or halo, grounded rail, one node accent); combined asymmetries accumulate a small right-lean.
  - Every other glyph (69/72 + `.notdef`) lands inside ±40. Whole-bank AVG |offset| = 8.58 units, MAX |offset| = 87.5 units. Tolerance of 100 accepts the three intentionally-asymmetric motifs with headroom while still catching a regression to `lsb=0` (which would produce effective offsets on the order of 300–500 units, well beyond 100).
- **Verification**: `python3 tools/font/build_font.py --check` → passes; `python3 tools/font/build_font.py --check-deterministic` → 7132 bytes, deterministic; `python3 scripts/verify-font.py` → 72 glyphs, advance 1000, WOFF2 7132 bytes, hmtx honesty + ink-center assertions all green; `node scripts/lint-sigils.js` → pass; `node scripts/verify-assets.js` → pass. Byte size 7132 (rev 2.1 was 7000; per-glyph LSB adds 72 short entries to hmtx and prevents the CFF-like compression of a uniform (1000, 0) run), still inside the 4–8 KB acceptance range with ~1 KB headroom.
- **Owner sign-off**: rev 2.2 supersedes the still-pending owner sign-off on the rev-2.1 contact sheet (control-and-polish SESSION-04). Re-review is requested at the same 34 px + 220 px scales; ink should now optically center within the cell of the contact sheet's `place-items: center` grid, and — because the canvas playfield uses `textAlign: 'center'` (`src/ui/playfield.js:60`) — party/enemy sigils should center in their lattice cells in-game as well.
- **Cache**: font bytes changed → `service-worker.js` `CACHE_VERSION` bumped to `2026-08-16-clarity-and-fit-v1` on top of the prior `2026-08-16-control-and-polish-v2`. SESSION-07 in this feature will bump to `-v2` after non-font clarity-and-fit changes land.
